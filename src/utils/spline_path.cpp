...
SplinePath::SplinePath(TankDrive *drive_system, pros::Imu *imu, pros::Motor *l_enc, pros::Motor *r_enc, motion_profile_t *motion_profile)
...
bool SplinePath::run_path(Waypoint *point_list, int list_length)
{
  if (run_path_init)
  {
    ...
    // Make sure the encoders are in their base "tick" units
    l_enc->set_encoder_units(pros::E_MOTOR_ENCODER_COUNTS);
    r_enc->set_encoder_units(pros::E_MOTOR_ENCODER_COUNTS);

    reset_heading = imu->get_rotation();
